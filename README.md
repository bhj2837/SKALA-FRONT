...
프레임워크도 빌드 도구도 쓰지 않았습니다. 순수 HTML / CSS / JavaScript입니다.
...
GitHub Pages 라면 저장소 **Settings → Pages** 에서
Source 를 `Deploy from a branch`, 브랜치를 `main` / `/ (root)` 로 두면 끝입니다.

몇 분 뒤 `https://<계정>.github.io/<저장소>/` 로 열립니다.
주소가 정해지면 각 페이지 `<head>` 에 `canonical` 과 `og:image` 를 채워 넣으면
링크 미리보기가 더 그럴듯해집니다. (지금은 주소를 몰라서 비워 둔 상태입니다)
...
코드는 마음껏 참고하셔도 됩니다.
다만 `media/` 안의 사진과 영상, 그리고 프로필 · 여행 기록 같은 글은
제 개인 기록이라 재사용 대상이 아닙니다.
